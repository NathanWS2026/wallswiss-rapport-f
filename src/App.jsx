...
function SlideDivider({ data, number, title }) {
  const fullName = `${data.prenom} ${(data.nom || "").toUpperCase()}`;
  return (
    <div style={{ ...slideBase, background: C.primaryDark, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ position: "absolute", bottom: 0, right: 0, width: "40%", height: "100%", background: `linear-gradient(to left, rgba(165,149,104,0.15), transparent)` }} />
      <div style={{ position: "absolute", top: 0, left: 0, width: 6, height: "100%", background: C.gold }} />
      
      <div style={{ zIndex: 2, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", padding: "0 40px" }}>
        <div style={{ width: 60, height: 2, background: C.gold, marginBottom: 24 }} />
        <div style={{ fontFamily: "'Times New Roman', Times, serif", color: C.white, fontSize: "clamp(32px, 4vw, 56px)", fontWeight: 700, lineHeight: 1.2, marginBottom: 32 }}>
          Solution d'investissement<br/>
          <em style={{ color: C.gold, fontStyle: "italic" }}>{title}</em>
        </div>
        <div style={{ fontFamily: "'Montserrat', sans-serif", fontStyle: "italic", color: "rgba(255,255,255,0.7)", fontSize: 13, maxWidth: 600, lineHeight: 1.6 }}>
...
function SlideContact({ data, editMode, onTextChange }) {
  const fullName = `${data.prenom} ${(data.nom || "").toUpperCase()}`;
  return (
    <div style={{ ...slideBase, background: `linear-gradient(135deg, ${C.primaryDark} 0%, ${C.black} 100%)`, display: "flex", flexDirection: "column" }}>
      <div style={{ position: "absolute", top: 0, right: 0, width: "50%", height: "100%", background: "rgba(255,255,255,0.02)" }} />
      <div style={{ position: "absolute", top: 0, left: 0, width: 4, height: "100%", background: C.gold }} />
      
      <div style={{ padding: "48px 56px", flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", position: "relative", zIndex: 2 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 60, alignItems: "center" }}>
          
          <div>
            <div style={{ borderLeft: `4px solid ${C.gold}`, paddingLeft: "16px", marginBottom: "28px" }}>
              <div style={{ fontSize: 10, color: C.gold, fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", marginBottom: 6 }}>SYNTHÈSE & CONTACT</div>
              <div style={{ fontFamily: "'Times New Roman', Times, serif", fontSize: 32, fontWeight: 700, color: C.white, margin: 0, lineHeight: 1.15 }}>
                Votre partenaire en <em style={{ color: C.gold }}>gestion de patrimoine</em>
              </div>
            </div>
            <EditableText editMode={editMode} value={data.texts?.contactDesc} onChange={v => onTextChange("contactDesc", v)} style={{ fontSize: 13, lineHeight: 1.9, color: "rgba(255,255,255,0.7)", margin: 0, maxWidth: 450 }} />
          </div>

          <div style={{ background: "rgba(255,255,255,0.05)", padding: "40px", border: `1px solid rgba(255,255,255,0.1)`, backdropFilter: "blur(10px)" }}>
            <div style={{ marginBottom: 30 }}>
              <img src={LOGO_URL} alt="WallSwiss" style={{ height: "64px" }} />
            </div>
            <div style={{ fontFamily: "'Times New Roman', Times, serif", fontSize: 28, fontWeight: 700, color: C.white }}>{data.conseiller || "Louis Borne"}</div>
            <div style={{ color: C.gold, fontSize: 12, fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase", marginTop: 8, marginBottom: 24 }}>{data.titreConseiller || "Conseillère en Gestion de Patrimoine"}</div>
            
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.8)", lineHeight: 2.2, fontWeight: 500 }}>
              <div style={{ display: "flex", gap: 12 }}><strong style={{ color: C.gold }}>T</strong> <span>{data.telephone || "+41.76.231.92.75"}</span></div>
              <div style={{ display: "flex", gap: 12 }}><strong style={{ color: C.gold }}>E</strong> <span>{data.email || "l.borne@wallswiss.ch"}</span></div>
              <div style={{ display: "flex", gap: 12 }}><strong style={{ color: C.gold }}>A</strong> <span>Rue Kleberg 14, 1201 Genève</span></div>
            </div>
          </div>
...
function ReportPreview({ data, onClose, onUpdateData }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.88)", zIndex: 200, overflow: "hidden", display: "flex", flexDirection: "column" }}>
      {/* Top bar */}
      ...
      {/* Slide area */}
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px 60px", position: "relative", minHeight: 0 }}>
      ...
      {/* Thumbnails */}
      <div style={{ background: C.black, padding: "8px 24px", display: "flex", gap: 4, overflowX: "auto", flexShrink: 0, borderTop: "1px solid rgba(255,255,255,0.08)" }}>
        {slides.map((_, i) => (
